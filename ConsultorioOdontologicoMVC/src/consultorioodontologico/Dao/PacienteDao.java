...
public class PacienteDao extends Pessoa {
    ...
    public ArrayList<Paciente> Pesquisar(String pesquisa) {
        ArrayList<Paciente> lista = new ArrayList<>();
            String sql = "SELECT Pac.idPaciente,pac.cod_pessoa, pes.nome, pes.cpf, pes.email, pes.endereco, pes.celular, pac.anamnese "
                + "FROM Paciente pac JOIN pessoa pes ON pac.cod_pessoa = pes.idPessoa WHERE nome LIKE '" + pesquisa + "%'";
        ResultSet rs = ModuloConexao.consultar(sql);
        if (rs != null) {
            try {
                while (rs.next()) {
